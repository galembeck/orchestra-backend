...
public class EmailService(IResend resend) : IEmailService
{
    ...
    private static string BuildGiftCardPurchasedTemplate(
        string name, string giftCardId, decimal amount, string paymentMethod)
    {
        var shortId = giftCardId.Length >= 8 ? giftCardId[..8].ToUpper() : giftCardId.ToUpper();
        var methodLabel = paymentMethod.ToUpper() switch
        {
            "PIX"     => "Pix",
            "BOLETO"  => "Boleto",
        ...
        var methodNote = paymentMethod.ToUpper() switch
        {
            "PIX"    => "Seu c&#243;digo Pix est&#225; dispon&#237;vel no aplicativo. O pagamento &#233; confirmado em instantes.",
            "BOLETO" => "Seu boleto est&#225; dispon&#237;vel no aplicativo. O prazo de vencimento &#233; de 3 dias &#250;teis.",
            _        => "Finalize o pagamento pelo aplicativo."
        };
            ...
            <tr>
              <td style="background:#ffffff;border-radius:8px 8px 0 0;">
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="background:#2c2825;padding:22px 32px;">
                      <table cellpadding="0" cellspacing="0">
                        <tr>
                          <td style="width:38px;height:38px;background:#e8a030;border-radius:50%;text-align:center;vertical-align:middle;font-size:18px;color:#fff;font-weight:700;line-height:38px;">&#9203;</td>
                          <td style="padding-left:14px;vertical-align:middle;">
                            <span style="font-size:16px;color:#f5ebe0;">Vale presente &mdash; </span>
                            <span style="font-size:16px;color:#c8a97e;font-weight:700;">aguardando pagamento</span>
                          </td>
                        </tr>
                      </table>
                    </td>
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="padding:32px 40px;">
                      <p style="margin:0 0 16px;font-size:15px;color:#2c2825;">Ol&#225;, <strong>{name}</strong>! Recebemos a sua solicita&#231;&#227;o de vale presente.</p>
                      <table width="100%" cellpadding="0" cellspacing="0" style="background:#faf7f4;border-left:3px solid #c8a97e;border-radius:0 8px 8px 0;margin-bottom:24px;">
                        <tr>
                          <td style="padding:20px 24px;">
                            <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;">
                              <tr>
                                <td style="padding:6px 0;color:#8a7b72;border-bottom:1px solid #ede8e2;">C&#243;digo do vale</td>
                                <td style="padding:6px 0;text-align:right;font-weight:700;border-bottom:1px solid #ede8e2;"><span style="background:#2c2825;color:#f5ebe0;padding:4px 12px;border-radius:20px;font-size:13px;">#{shortId}</span></td>
                              </tr>
                              <tr>
                                <td style="padding:6px 0;color:#8a7b72;border-bottom:1px solid #ede8e2;">Valor</td>
                                <td style="padding:6px 0;text-align:right;font-weight:700;border-bottom:1px solid #ede8e2;">R$ {amount:N2}</td>
                              </tr>
                              <tr>
                                <td style="padding:6px 0;color:#8a7b72;border-bottom:1px solid #ede8e2;">Forma de pagamento</td>
                                <td style="padding:6px 0;text-align:right;font-weight:700;border-bottom:1px solid #ede8e2;">{methodLabel}</td>
                              ...
                              <tr>
                                <td style="padding:6px 0;color:#8a7b72;">Status</td>
                                <td style="padding:6px 0;text-align:right;font-weight:700;color:#e8a030;">Aguardando pagamento</td>
                              </tr>
                            </table>
                          </td>
                        </tr>
                      ...
                      <p style="margin:0;font-size:13px;color:#a0908a;">Assim que o pagamento for confirmado, voc&#234; receber&#225; um novo e-mail com seu vale presente ativo. 🎁</p>
                    </td>
                  </tr>
                </table>
              </td>
        ...
        return WrapLayout(
            $"Recebemos seu pedido de vale presente de R$ {amount:N2}. Confirme o pagamento para ativ&#225;-lo.",
            body);
    }

    private static string BuildGiftCardActivatedTemplate(
        string name, string giftCardId, decimal amount, DateTime expiresAt)
    {
        var shortId   = giftCardId.Length >= 8 ? giftCardId[..8].ToUpper() : giftCardId.ToUpper();
        var expiryStr = expiresAt.ToString("dd/MM/yyyy");
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="background:#2c2825;padding:22px 32px;">
                      <table cellpadding="0" cellspacing="0">
                        <tr>
                          <td style="width:38px;height:38px;background:#c8a97e;border-radius:50%;text-align:center;vertical-align:middle;font-size:20px;line-height:38px;">&#127873;</td>
                          <td style="padding-left:14px;vertical-align:middle;">
                            <span style="font-size:16px;color:#f5ebe0;">Vale presente ativado &mdash; </span>
                            <span style="font-size:16px;color:#c8a97e;font-weight:700;">pronto para uso!</span>
                          </td>
                        </tr>
                      </table>
                    </td>
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="padding:32px 40px 24px;">
                      <p style="margin:0 0 20px;font-size:15px;color:#2c2825;">Que &#243;tima not&#237;cia, <strong>{name}</strong>! Seu vale presente est&#225; ativo e pronto para ser utilizado.</p>
                      <!-- Amount highlight -->
                      <table width="100%" cellpadding="0" cellspacing="0" style="background:#2c2825;border-radius:10px;margin-bottom:24px;">
                        <tr>
                          <td style="padding:28px;text-align:center;">
                            <p style="margin:0 0 6px;font-size:11px;color:#c8a97e;letter-spacing:3px;text-transform:uppercase;">Valor do vale</p>
                            <p style="margin:0 0 10px;font-size:36px;font-weight:700;color:#f5ebe0;">R$ {amount:N2}</p>
                            <p style="margin:0;font-size:13px;color:#c8a97e;letter-spacing:1px;">C&#243;digo: <strong style="color:#f5ebe0;letter-spacing:3px;">#{shortId}</strong></p>
                          </td>
                        </tr>
                      </table>
                      <!-- Info box -->
                        ...
                        <tr>
                          <td style="padding:20px 24px;">
                            <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;">
                              <tr>
                                <td style="padding:6px 0;color:#8a7b72;border-bottom:1px solid #ede8e2;">Status</td>
                                <td style="padding:6px 0;text-align:right;font-weight:700;color:#4caf50;border-bottom:1px solid #ede8e2;">Ativo</td>
                              </tr>
                              <tr>
                                <td style="padding:6px 0;color:#8a7b72;border-bottom:1px solid #ede8e2;">Valor dispon&#237;vel</td>
                                <td style="padding:6px 0;text-align:right;font-weight:700;border-bottom:1px solid #ede8e2;">R$ {amount:N2}</td>
                              </tr>
                              <tr>
                                <td style="padding:6px 0;color:#8a7b72;">V&#225;lido at&#233;</td>
                                <td style="padding:6px 0;text-align:right;font-weight:700;">{expiryStr}</td>
                              </tr>
                            </table>
                          </td>
                        </tr>
                      ...
                      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
                        <tr>
                          <td style="background:#fdf6ec;border:1px solid #e8c99a;border-radius:6px;padding:12px 16px;font-size:13px;color:#7a5c2e;line-height:1.6;">
                            &#128204; Para utilizar, selecione <strong>Vale Presente</strong> como forma de pagamento ao finalizar sua compra na loja.
                          </td>
                        </tr>
                      </table>
                    </td>
        ...
        return WrapLayout(
            $"Seu vale presente Terra & Tallow de R$ {amount:N2} está ativo. Use até {expiryStr}.",
            body);
    }
